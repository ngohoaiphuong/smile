...
$(function() {
  sdc.formHelpers.initFieldContainers = function($context) {
    ...
    $('.trimmed-and-squished-field').on('change', squishAndTrimField);
    ...
    function squishAndTrimField() {
      this.value = this.value.replace(/(^\s*)|(\s*$)/gi, ""); // removes leading and trailing spaces
      this.value = this.value.replace(/\s+/g, ' '); // squish multiple sequential whitespace characters into a single whitespace character
    }
