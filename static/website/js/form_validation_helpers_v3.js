...
(function () {
  ...
  const minimumAge = window.minimumAge || 12;
  ...
  customFields.sepaDebitIbanNumber = {
    inputMask: makeInputMask('AA99 9999 9999 9999 99999{0,2}'),
    regex: /^((AT|at)[0-9]{2} [0-9]{4} [0-9]{4} [0-9]{4} [0-9]{4}|((DE|de)[0-9]{2} )[0-9]{4} [0-9]{4} [0-9]{4} [0-9]{4}[0-9]{2})$/,
  };
  ...
  window.Parsley.addValidator('customerAge', {
    ...
    messages: {
      en: interpolate(gettext("Minimum Customer age is %s."), [consentAge]),
      local: interpolate(gettext("Minimum Customer age is %s."), [consentAge])
    },
  });
  ...
  window.Parsley.addValidator('guardianAge', {
    ...
    // Template literals are not supported yet by easygettext
    messages: {
      en: interpolate(gettext("Minimum Guardian age is %s."), [guardianConsentAge]),
      local: interpolate(gettext("Minimum Guardian age is %s."), [guardianConsentAge]),
    },
  });
  ...
  window.Parsley.addValidator('isYear', value => {
    ...
    const end_year = curr_year - minimumAge;
    if (year_int < 1900) {
    ...
    if (year_int > end_year) {
      window.Parsley.addMessage('en', 'isYear', interpolate(gettext('Minimum age requirement is %s.'), [minimumAge]));
      window.Parsley.addMessage('local', 'isYear', interpolate(gettext('Minimum age requirement is %s.'), [minimumAge]));
      return false;
    }
    return true;
  });
  ...
  window.Parsley.addValidator('isAscii', {
    ...
    validateString(value) {
      ...
      if (!emailRegex.test(value)) {
        console.error('Invalid email', {'message': 'This field cannot contain special characters.'});
        for (let i = 0; i < email.length; i++) {
          ...
          if (!/^[a-zA-Z0-9!#$%^&*_+\-=?.~]*$/.test(email[i])) {
            ...
            const message = interpolate(format, [value, emailMatches]);
    ...
    window.Parsley.addValidator('patientAge', (value, requirement) => {
      ...
      if (userIsPatient && patientAge < consentAge) {
        return setUIandReturn(
          false,
          interpolate(
            gettext("If you\'re under %s, you cannot register. Please select \"this order is for somebody else\" to continue."),
            [consentAge]
          )
        );
      }
      if (patientAge < minimumAge) {
        return setUIandReturn(false, interpolate(gettext('Minimum age requirement is %s.'), [minimumAge]));
      }
      if (patientAge < consentAge && !userIsGuardian) {
        return setUIandReturn(
          false,
          interpolate(
            gettext("If the patient is under %s, you need to be the guardian to continue."),
            [consentAge]
          )
        );
      }
  ...
  (function () {
    window.Parsley.addValidator('notequalto', (value, requirement) => {
      ...
      if ($patientFirstName.val().toLowerCase().trim() + $patientLastName.val().toLowerCase().trim() == $guardianFirstName.val().toLowerCase().trim() + $guardianLastName.val().toLowerCase().trim()) {
        return setUIandReturn(false, gettext('The guardian’s first and last name cannot be the same as the patient.'));
      }
      return setUIandReturn(true);
  ...
  window.Parsley.addValidator('validPhone', {
    ...
    messages: {
      en: gettext('Please enter a valid phone number.'),
